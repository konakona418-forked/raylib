...
#include <sys/stat.h>               // Required for: stat(), S_ISREG [Used in GetFileModTime(), IsFilePath()]
    ...
    #include <sys/types.h>          // Required for: pid_t [Used in Process struct definition]
#endif
...
//----------------------------------------------------------------------------------
// Module Functions Definition: Process Management
//----------------------------------------------------------------------------------
...
RLAPI Process InitProcess(const char *command, char *const args[])
{
    ...
    else if (pid == 0)
    {
        ...
        TRACELOG(LOG_WARNING, "PROCESS: Failed to execute process command");
        exit(1);
    }
    else
    {
